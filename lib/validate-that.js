...
(function () {
  ...
  (function() {
    ...
    function obtainValue(data, prop) {
      var propPath = prop.split("."); 
      var value = data;
      for (var i = 0; i < propPath.length; i++) {
        var propStep = propPath[i];
        if (value === undefined || value === null) break;
        value = value[propStep];
      }
      return value
    }

    function handlePropertyChecking(check, prop) {
      var value = obtainValue(this.data, prop);
      var result = check(value);
      if (!result) putError(check, value, prop, check.checkArgs);
      return result;
    }
